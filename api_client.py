...
class MOEXClient:
    ...
    def get_top_stocks(_self) -> pd.DataFrame:
        """Fetch top 30 stocks from MOEX"""
        url = f"{_self.BASE_URL}/iss/engines/stock/markets/shares/boards/tqbr/securities.json"
    ...
    def get_stock_info(_self, ticker: str) -> Dict:
        ...
        url = f"{_self.BASE_URL}/iss/engines/stock/markets/shares/boards/tqbr/securities/{ticker}.json"
        response = requests.get(url)
        response.raise_for_status()
    ...
    @st.cache_data(ttl=300)
    def get_candles(_self, ticker: str, days: int = 30) -> pd.DataFrame:
        """Fetch candle data for a specific stock"""
        from_date = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d")
        till_date = datetime.now().strftime("%Y-%m-%d")

        url = f"{_self.BASE_URL}/iss/engines/stock/markets/shares/boards/tqbr/securities/{ticker}/candles.json"
        params = {
            "from": from_date,
            "till": till_date,
